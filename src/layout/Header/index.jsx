...
const Header = () => {
	...
	return (
		<header
			className={`header bg-white py-4  relative mb-8 ${
				mobileNav ? 'shadow-md' : 'shadow-sm'
			}`}
		>
			{toggleSearch && (
				<Search
			...
			<nav className='flex items-center justify-between container mx-auto md:items-start lg:items-center md:px-4 flex-col md:flex-row pt-16 md:pt-0 lg:pt-0'>
				<ul
					className={`lg:flex items-center gap-11 md:flex-col lg:flex-row  flex-col justify-center z-20 ${
						mobileNav ? '' : 'hidden'
					}`}
				>
					{routes.map((route) => (
				...
				<ul
					className={`items-center z-20  gap-9 lg:flex md:flex  justify-center mt-4 md:mt-0 lg:mt-0  pt-3 md:pt-0 lg:pt-0 mx-4 md:border-none lg:border-none w-full md:w-auto lg:w-auto ${
						mobileNav ? '' : 'hidden'
					}`}
				>
